...
export function AppCard({ app, index, adminMode, onDelete }: AppCardProps) {
  const handleClick = () => {
    if (!adminMode) {
  ...
  return (
    <motion.div
      ...
      className={`bento-card group flex flex-col items-center justify-center gap-4
        ...
        ${adminMode ? "ring-2 ring-dashed ring-surface-200" : ""}`}
    >
      {adminMode && (
        <motion.button
          ...
          className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center
            rounded-xl bg-red-50 text-red-500 transition-colors hover:bg-red-100"
        >
      ...
      {adminMode && (
        <div className="absolute left-3 top-3 text-surface-300">
      ...
      <div className="relative">
        <div
          className="flex h-16 w-16 items-center justify-center rounded-2xl bg-surface-50
      ...
      <div className="text-center">
        <h3 className="text-sm font-semibold text-surface-800">{app.name}</h3>
        {app.category && (
          <span className="mt-1 inline-block text-[11px] font-medium text-surface-400">
            {app.category}
          </span>
        )}
      </div>
      ...
      {!adminMode && (
        ...
        >
          <ExternalLink className="h-3.5 w-3.5 text-surface-300" />
        </div>
      )}
    </motion.div>
  );
